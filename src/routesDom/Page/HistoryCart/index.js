...
export default function HistoryCart() {
    ...
    const actionPutCartStatusOrderAPI = (data, token) =>
        dispatch(putCartStatusOrderAPI(data, token));
    const actionPutCartAddressesAPI = (data, token) =>
        dispatch(putCartAddressesAPI(data, token));
    const actionDeleteCartAPI = (id_card, token) =>
        dispatch(deleteCartAPI(id_card, token));
    // store
    const dataHistoryCart = useSelector((state) => state.cart.historyCart);
    const loadingUpdateCartStatus = useSelector(
        (state) => state.cart.loadingUpdateCartStatus
    );
    const loadingHistoryCart = useSelector(
        (state) => state.cart.loadingHistoryCart
    );
    const loadingDeleteCartAPI = useSelector(
        (state) => state.cart.loadingDeleteCartAPI
    );
    // useEffect
    useEffect(() => {
        window.scrollTo({
    ...
    return token ? (
        <div className="group-history-cart">
            <div className="container-history-cart">
                ...
                <h3>
                    LỊCH SỬ MUA HÀNG
                    <span>({dataHistoryCart.length} sản phẩm)</span>
                </h3>
