...
export default function DetailProducts({ location }) {
    ...
    const page_limit =
        Number(queryString.parse(location.search).page_limit) || 1;
    const page_cmt = Number(queryString.parse(location.search).page_cmt) || 1;
    let historyProduct =
        JSON.parse(localStorage.getItem("history_product")) || [];
    // state default
    ...
    {
        dataProductsId &&
            (document.querySelector("title").innerHTML = dataProductsId.name);
    }
    ...
    const showHistoryProduct = () => {
        ...
        localStorage.setItem(
            "history_product",
            JSON.stringify(historyProductOld)
        );
    };
    //  get one product
                    ...
                    <div className="container-detail-products">
                        <div className="group-detail">
                            ...
                            <div className="link-group">
                                ...
                                <Link
                                    to={`/product?trademark=${dataProductsId.key.replace(
                                        / /g,
                                        "-"
                                    )}`}
                                >
                                ...
                                <Link
                                    to={`/product-type?nsx=${dataProductsId.NSX.replace(
                                        / /g,
                                        "-"
                                    )}`}
                                >
                            ...
                            {isAdmin && token && (
                                <div className="ground-btn-admin">
                                    ...
                                    <Button type="primary">
                                        <Link
                                            to={`/admin-edit-product/${id_product}`}
                                        >
                                            {" "}
                                            Chỉnh Sữa
                                        </Link>
