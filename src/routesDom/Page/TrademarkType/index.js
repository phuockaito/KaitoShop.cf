...
export default function TrademarkType({ location }) {
    ...
    document.querySelector("title").innerHTML = nsx
        .replace(/-/g, " ")
        .toUpperCase();
    const page = Number(queryString.parse(location.search).page) || 1;
    const sort_price =
        Number(queryString.parse(location.search).sort_price) || 0;
    ...
    const loadingTrademarkType = useSelector(
        (state) => state.trademarkType.loading
    );
    const lengthTrademarkType = useSelector(
        (state) => state.trademarkType.length
    );
    // dispatch Api
    const actionGetProductTrademarkType = (params) =>
        dispatch(getProductTrademarkType(params));
    //useEffect
    const fetchAPi = () => {
        const params = {
            page: page,
            ...
            <div className="container-products-nsx">
                <div className="products-nsx">
                    <div className="group-products-nsx">
                        ...
                        {!loadingTrademarkType &&
                            dataTrademarkType.length > 0 && (
