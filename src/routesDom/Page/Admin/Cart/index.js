...
import {
    ...
    messagesCart,
    getCart,
} from "features/Admin/Cart/pathAPI";
// component
...
export default function ListProduct() {
    ...
    const loadingDeleteCartAPI = useSelector(
        (state) => state.cartAdmin.loadingDeleteCartAPI
    );
    ...
    // dispatch API
    const actionCheckOutCart = (id_cart, token) =>
        dispatch(checkOutCart(id_cart, token));
    const actionDeleteCart = (id_cart, token) =>
        dispatch(deleteCart(id_cart, token));
    const actionMessagesCart = (data, token) =>
        dispatch(messagesCart(data, token));
                    ...
                    {!loading &&
                        data.map((cartItems, index) => (
                            <div className="cart-items" key={index}>
                                <CartItem data={cartItems.cart} key={index} />
                                <CartInForBuy
                                    cart={cartItems.cart}
