...
import {
    ...
    deleteToProduct,
    getCommentProduct,
} from "features/Admin/Product/pathAPI";
...
export default function ProductManagement() {
    ...
    const loadingDelete = useSelector(
        (state) => state.productAdmin.loadingDelete
    );
    // comment product
    const commentProduct = useSelector(
        (state) => state.productAdmin.commentProduct
    );
    const lengthCommentProduct = useSelector(
        (state) => state.productAdmin.lengthCommentProduct
    );
    const loadingGetComment = useSelector(
        (state) => state.productAdmin.loadingGetComment
    );
    //dispatch API
    const actionGetListProduct = (params) => dispatch(getListProduct(params));
    const actionGetCommentProduct = (params, token) =>
        dispatch(getCommentProduct(params, token));
    const actionDeleteProduct = (id, token) =>
        dispatch(deleteToProduct(id, token));
    const actionDeleteCommentUser = (params, token) =>
        dispatch(deleteCommentUser(params, token));
    //useEffect
    useEffect(() => {
        window.scrollTo({
            top: 0,
    ...
    const Columns = [
        {
            ...
            render: (product) => (
                <Tooltip Tooltip placement="topLeft" title={product.name}>
                    ...
                    >
                        <Tag color="#f5222d" key={product.name}>
                            {product.name}
                        </Tag>
                    </Link>
                </Tooltip>
