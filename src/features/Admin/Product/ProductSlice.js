    ...
    extraReducers: {
        ...
        [deleteCommentUser.fulfilled]: (state, action) => {
            ...
            const indexProduct = productData.findIndex(
                (prod) => prod.product._id === id_product
            );
            const indexUser = commentProduct.findIndex(
                (comment) => comment._id === id_comment
            );
            if (indexProduct !== -1) {
                productData[indexProduct].length_comment =
                    productData[indexProduct].length_comment - 1;
            }
            if (indexUser !== -1) {
